...
class Executor(Generic[StateT]):
    ...
    def _persist_interrupt(
        ...
        # Ensure the persisted checkpoint also contains the events that led to
        # the interruption (node.interrupted and related messages).
        try:
            checkpoint.events = list(run.events)
            checkpointer.save(checkpoint)
        except Exception:
            # Best-effort persistence; do not mask the interrupt.
            pass
